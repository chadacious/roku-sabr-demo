' Auto-generated encoder/decoder for samples.UnpackedEnumMessage

function UnpackedEnumMessageEncode(message as Object) as String
    ...
    if choicesItems <> invalid then
        for each choicesItem in choicesItems
            numericValue = UnpackedEnumMessage_choices_normalizeEnum(choicesItem)
...
function UnpackedEnumMessage_choices_normalizeEnum(value as Dynamic) as Integer
    values = UnpackedEnumMessage_choices_getEnumValues()
...
function UnpackedEnumMessage_choices_enumName(value as Integer) as Dynamic
    names = UnpackedEnumMessage_choices_getEnumNames()
    key = StrI(value, 10)
...
function UnpackedEnumMessage_choices_getEnumValues() as Object
    globalAA = GetGlobalAA()
    key = "UnpackedEnumMessage_choices_EnumValues"
...
function UnpackedEnumMessage_choices_getEnumNames() as Object
    globalAA = GetGlobalAA()
    key = "UnpackedEnumMessage_choices_EnumNames"
    if globalAA <> invalid and globalAA.DoesExist(key) then
        return globalAA[key]
    end if
    table = {}
...
function UnpackedEnumMessageDecode(encoded as String) as Object
    ...
    while cursor < limit
        ...
        if fieldNumber = 1 then
            ...
            if wireType = 0 then
                ...
                choicesValues.Push(UnpackedEnumMessage_choices_enumName(numericValue))
            else
